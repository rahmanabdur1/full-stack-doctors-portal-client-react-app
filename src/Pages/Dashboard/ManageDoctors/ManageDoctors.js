...
const ManageDoctors = () => {
    ...
    const handleDeleteDoctor =doctor =>{
         fetch(`https://full-doctors-portal-server-code.vercel.app/doctors/${doctor._id}`,{
            method: 'DELETE',
            headers:{
    ...
    const { data: doctors, isLoading,refetch } = useQuery({
        ...
        queryFn: async () => {
            try {
                const res = await fetch('https://full-doctors-portal-server-code.vercel.app/doctors', {
                    headers: {
                        authorization: `bearer ${localStorage.getItem('accessToken')}`
                    }
                });
